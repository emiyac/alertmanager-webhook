...
# 修改uvicorn默认的日志格式
LOGGING_CONFIG["formatters"]["default"]["fmt"] = "%(asctime)s %(name)s %(threadName)s - %(levelprefix)s %(message)s"
LOGGING_CONFIG["formatters"]["access"][
    "fmt"] = '%(asctime)s %(name)s %(threadName)s %(levelprefix)s %(client_addr)s - "%(request_line)s" %(status_code)s'
...
class Settings(BaseSettings):
    ...
    DINGTALK_TEMPLATE: str = Field(default="dingtalk.tmpl", env="DINGTALK_TEMPLATE", description="默认的钉钉告警信息模板")
